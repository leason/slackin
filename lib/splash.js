...
export default function splash({ path, name, org, coc, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('form',
      ...
      dom('input.form-item type=email placeholder=you@ipreo/epam.com '
        + (!iframe ? 'autofocus' : '')),
      coc && dom('.coc',
        dom('div', {class:'cocHighPoints'}, 
          dom('b', 'Code of Conduct High Points:'),
          dom('ul',
            dom('li','No client confidential information unless necessary to conduct Ipreo business'),
            dom('li','No explicit or offensive photos (Cats are fine, though)')
          )
        ),
        dom('label',
...
function style({ logo, active, iframe } = {}){
  ...
  css.add('div.cocHighPoints', {
    'text-align':'left',
    'font-weight':'bold'
  });
  css.add('div.cocHighPoints LI', {
    'padding':'0 0 5px 0'
  });
